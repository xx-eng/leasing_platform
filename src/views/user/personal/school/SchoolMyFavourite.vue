    ...
    <div class="goods">
      ...
      <div class="goods-box">
        <div class="goods-item" v-for="item in goods" :key="item.id" @click="goToDetail(item)">
          <div class="show-all"><img :src="item.image" alt=""/></div>
  ...
  components: {
    ...
    mounted() {
      getFavorites(user.state.userId).then(res => {
        this.goods = res.data
        this.goods = this.goods.filter(goods => goods.stuGoods)
    ...
    methods: {
      goToDetail(item){
        this.$router.push({path: '/school/schoolGoodsDetail/'+item.goodsId})
      }
    }
  }
</script>
